...
The following derived files are present. The six foundation files
...
| `src/WinDayFlow.Capture.Native/internal/capture_policy.h` | `windows/runner/capture_runtime.h` | `134EEC71A54F0AE13EFC24045A97360221401FB5611E950BE1C92476B036752D` | `WORKTREE (pending commit)` | Added frame-only cadence reanchoring so each chunk's first successfully encoded frame establishes the next sample without changing the independent context cadence. |
| `src/WinDayFlow.Capture.Native/internal/capture_policy.cpp` | `windows/runner/capture_runtime.cpp` | `0285D25EB24D9D81495226DD2DC0FF055ECA9A1E336BBE5AF5FA507DB8E0B132` | `WORKTREE (pending commit)` | Added overflow-safe frame-only deadline reanchoring while preserving the independent context schedule and existing no-burst behavior. |
...
| `src/WinDayFlow.Capture.Native/tests/capture_policy_tests.cpp` | `windows/runner/capture_runtime_test.cpp` | `F013F0005A5E07D32BF02B41FDFDCA652003AAAC64E3B73EC49B3988CB23610A` | `WORKTREE (pending commit)` | Added monotonic context-cadence preservation and `INT64_MAX` exhaustion coverage for per-chunk frame reanchoring. |
| `src/WinDayFlow.Capture.Native/internal/atomic_chunk_store.h` | `windows/runner/capture_service.cpp` | `2EFB508275D7F183B774F310DB9CB5601D2033181689E8A8835E7F111715CD9E` | `b18a5df60e9af4b464c291043f2609a308ea2360` | Replaced per-file publication with a typed-manifest, bounded whole-directory transaction and an acknowledgement-controlled, retryable compensation handle. |
| `src/WinDayFlow.Capture.Native/internal/atomic_chunk_store.cpp` | `windows/runner/capture_service.cpp` | `9F9F0EBCD34E8769396152A57E9AE275FC31D0929F4C52F2D050348EE35437CB` | `b18a5df60e9af4b464c291043f2609a308ea2360` | Added no-follow identity locks for the local directory chain, same-volume staging, exact flushes, handle-bound no-overwrite rename, and observable retryable rollback before event acknowledgement. |
| `src/WinDayFlow.Capture.Native/internal/chunk_manifest.h` | `windows/runner/capture_service.cpp` | `D3BF80F1F359C0E8B3EA904E10588D034B85780F2C720BA4895A7C5FE247E045` | `WORKTREE (pending commit)` | Keeps metadata closed and privacy-safe while distinguishing foreground-scoped evidence from user-authorized display-wide continuous evidence. |
| `src/WinDayFlow.Capture.Native/internal/chunk_manifest.cpp` | `windows/runner/capture_service.cpp` | `2FEF2FAC5523B4C3206023948A3C63FC536C4C666CAC23D9781E5B459AC99119` | `WORKTREE (pending commit)` | Emits only validated timing, video, authorization generations, and one bounded authorization-scope label; no raw window, process, handle, or display identity is persisted. |
| `src/WinDayFlow.Capture.Native/internal/dxgi_desktop_frame_source.h` | `windows/runner/capture_service.cpp` | `96DFE4CC5CB7D9AC849015D007B3B50841927E3405C1E9F8F6BD50A14A9CA89C` | `WORKTREE (pending commit)` | Split the display-bound Desktop Duplication source from lifecycle and persistence concerns, with explicit 8K/BGRA bounds, a distinct access-denied result for controlled WGC fallback, and fail-closed outcomes. |
| `src/WinDayFlow.Capture.Native/internal/dxgi_desktop_frame_source.cpp` | `windows/runner/capture_service.cpp` | `1C6F70182B11FC040C7E16A6151AF7BF3A650435E85EA8E08A47CE31A081BCEB` | `WORKTREE (pending commit)` | Uses only the strict resolver, revalidates the full fingerprint, bounds GPU/CPU geometry, distinguishes access denial from transient device loss, orders Unmap/ReleaseFrame cleanup, never reuses timeout pixels, and safely copies pitched/rotated BGRA data. |
| `src/WinDayFlow.Capture.Native/internal/mf_h264_chunk_writer.h` | `windows/runner/capture_service.cpp` | `E93746B6A4F1824FAADBF0DB75F8BA8B3D633CA5613E9289D0FEFEB89329E913` | `b18a5df60e9af4b464c291043f2609a308ea2360` | Replaced path-backed sink ownership with an explicit same-thread, bounded, in-memory H.264 writer state machine. |
| `src/WinDayFlow.Capture.Native/internal/mf_h264_chunk_writer.cpp` | `windows/runner/capture_service.cpp` | `657F58428EF22BDBF718C9FE103D5A9AFBB74E836B9E6B8AE61AA897A3953529` | `b18a5df60e9af4b464c291043f2609a308ea2360` | Added a 64 MiB fail-closed COM stream, paired COM/MF lifetime, strict BGRA samples, final-frame timing, secure overflow discard, and no temporary video file. |
| `src/WinDayFlow.Capture.Native/internal/wic_bgra_scaler.h` | `windows/runner/capture_service.cpp` | `2AE028EB5AE7E1B1BBBB3338716657D84693D9AC40A63E946C7B17F9125BA874` | `b18a5df60e9af4b464c291043f2609a308ea2360` | Isolated bounded BGRA scaling and even H.264 dimension calculation behind an internal WIC contract. |
| `src/WinDayFlow.Capture.Native/internal/wic_bgra_scaler.cpp` | `windows/runner/capture_service.cpp` | `9849E326507839C87AE30A4D707CA7CEE5EF7E40A2F90E08D402CDA3FC33570C` | `b18a5df60e9af4b464c291043f2609a308ea2360` | Prevents upscaling, validates exact byte geometry and UINT bounds, preserves aspect ratio, and normalizes output alpha. |
